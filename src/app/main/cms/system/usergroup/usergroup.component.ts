...
@Component({
  selector: "cms-app-usergroup",
  templateUrl: "./usergroup.component.html",
  styleUrls: ["./usergroup.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class SysUserGroupComponent implements OnInit {
  // Varriable Language
  languages: any;
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "get-list-usergroup", extraParams);
  };
  public dataStateChange(state: DataStateChangeEventArgs): void {
    this.state = state;
    this.pageIndex = Math.floor(state.skip / state.take);
    this.pageSize = Math.round(state.take);
    let extraParams = [];
    if (this.search.text) {
      extraParams.push({
        field: "text",
    ...
    this._coreService.execute(state, "get-list-usergroup", extraParams);
  }
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/system/sysusergroup/new"]);
        break;
      default:
        break;
    }
  ...
  clickRecord = (data, status) => {
    if (data && status === 'view') {
      ...
      const paramAdd = window.btoa(JSON.stringify(objParamAdd));
      // window.open("/cms/system/sysusergroup/" + paramAdd, "_blank");
      this.router.navigate(["/cms/system/sysusergroup", paramAdd]);
