...
export class SysUserGroupEditComponent implements OnInit {
  paramId: number;
  // Varriable Language
  flagState = "";
  model: UserGroup = new UserGroup();
  modelOrigin: UserGroup = new UserGroup();
  // language
  languages: any;
  ...
  getDetailData = () => {
    this._coreService.Get('get-usergroup/' + this.paramId).subscribe(data => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.BACK:
        if (this.editForm.dirty && this.editForm.touched) {
          ...
          this.router.navigate(["/cms/system/sysusergroup"]);
        }
        break;
      default:
        break;
  ...
  saveData() {
    if (!this.editForm.valid) {
      ...
      if (this.flagState === 'new') {
        ...
        this._coreService
          .Post("create-usergroup", objApiAdd)
          .subscribe((res) => {
            ...
            if (x && x.errorCode == "ERROR_CODE_EXISTS") {
              this.notification.warning(
                "Mã nhóm đã tồn tại trong hệ thống. Vui lòng nhập lại!"
              );
            } else if(x.errorCode === 11000) {
              this.notification.warning(
                "Tên nhóm tài khoản đã tồn tại trong hệ thống. Vui lòng nhập lại!"
              );
            } else {
      ...
      if (this.flagState === 'edit') {
        ...
        this._coreService
          .Post("update-usergroup", objApiEdit)
          .subscribe((res) => {
