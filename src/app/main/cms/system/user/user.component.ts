...
export class SysUserComponent implements OnInit {
  ...
  viewRecord = (e) => {
    ...
    window.open("cms/system/sysuser/" + paramAdd, "_blank");
  };
  ...
  getListData = (): void => {
    ...
    if (this.search.text) {
      extraParams.push({
        field: "text",
        value: this.search.text.trim(),
      });
    }
    if (this.gridInstance) {
      this.gridInstance.goToPage(1);
    }
    this.systemUserService.execute(this.state, "get-list-user", extraParams);
  };
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    this.systemUserService.execute(state, "get-list-user", extraParams);
  }
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/system/sysuser/new"]);
        break;
      default:
        break;
    }
  ...
  clickRecord = (data, status) => {
    if (data && status === 'view') {
      ...
      const paramAdd = window.btoa(JSON.stringify(objParamAdd));
      // window.open("/cms/system/sysuser/" + paramAdd, "_blank");
      this.router.navigate(["/cms/system/sysuser", paramAdd]);
