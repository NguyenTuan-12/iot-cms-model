...
export class BlogCategoryComponent implements OnInit {
          ...
          .subscribe((success) => {
            ...
            if (x && x.errorCode == "ERROR_CATEGORY_HAS_DATA") {
              this.notification.warning(
                "Không được phép xóa nhóm tin tức đã tồn tại danh sách tin tức!"
              );
            } else {
              this.notification.deleteError();
            }
  ...
  clickRecord = (data, status) => {
    if (data && status === 'view') {
      ...
      const paramAdd = window.btoa(JSON.stringify(objParamAdd));
      // window.open("/cms/blog/blogcategory/" + paramAdd, "_blank");
      this.router.navigate(["/cms/blog/blogcategory", paramAdd]);
    }
    if (data && status === 'edit') {
      this.modelAdd = data;
