...
export class ProductCategoryEditComponent implements OnInit {
  ...
  getDetailData = () => {
    this._coreService
      .Get("get-productcategory/" + this.paramId)
      .subscribe((data) => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.BACK:
        if (this.editForm.dirty && this.editForm.touched) {
          ...
          this.router.navigate(["/cms/productservice/productcategory"]);
        }
        break;
      default:
        break;
  ...
  saveData() {
    if (!this.editForm.valid) {
      ...
      if (this.flagState === "new") {
        ...
        this._coreService.Post("create-productcategory", objApiAdd).subscribe(
          (res) => {
          ...
          (error) => {
            ...
            if (x && x.errorCode == "ERROR_CODE_EXISTS") {
              this.notification.warning(
                "Mã nhóm/ Tên nhóm sản phẩm đã tồn tại trong hệ thống. Vui lòng nhập lại!"
              );
            } else {
              this.notification.addError();
            }
      ...
      if (this.flagState === "edit") {
        ...
        this._coreService.Post("update-productcategory", objApiEdit).subscribe(
          (res) => {
