...
@Component({
  selector: "cms-app-productcategory",
...
export class ProductCategoryComponent implements OnInit {
  ...
  viewRecord = (e) => {
    ...
    window.open("cms/system/sysusergroup/" + paramAdd, "_blank");
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "get-list-productcategory", extraParams);
  };
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    this._coreService.execute(state, "get-list-productcategory", extraParams);
  }
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/productservice/productcategory/new"]);
        break;
      default:
        break;
    }
      ...
      if (lstDeleteIds && lstDeleteIds.length > 0) {
        this._coreService
          ...
          .subscribe((success) => {
            ...
            if(data.errorCode=="ERROR_CATEGORY_HAS_DATA"){ 
              this.notification.warning("Không được phép xóa nhóm sản phẩm đã tồn tại trong danh sách sản phẩm!");
              
            }else{
              this.notification.warning("Xóa không thành công");
            }
  ...
  clickRecord = (data, status) => {
    if (data && status === 'view') {
      ...
      const paramAdd = window.btoa(JSON.stringify(objParamAdd));
      // window.open("/cms/productservice/productcategory/" + paramAdd, "_blank");
      this.router.navigate(["/cms/productservice/productcategory", paramAdd]);
