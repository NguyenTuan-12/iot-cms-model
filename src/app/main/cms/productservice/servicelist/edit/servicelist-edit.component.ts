...
@Component({
  selector: "app-servicelist-edit",
  templateUrl: "./servicelist-edit.component.html",
  styleUrls: ["./servicelist-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class ServiceListEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    async.waterfall([
      (cb)=>{
        this._coreService.Get("get-combo-service-category").subscribe(
          (res:any)=>{
            if(res.status ==200){
  ...
  getDetailData = () => {
    this._coreService.Get('get-service/' + this.paramId).subscribe(data => {
    ...
    switch (buttonId) {
      ...
      case ToolbarItem.BACK:
        if (this.editForm.dirty && this.editForm.touched) {
          ...
          this.router.navigate(["/cms/productservice/servicelist"]);
        }
        break;
      default:
        break;
  ...
  saveData() {
    if (!this.editForm.valid) {
      ...
      if (!this.slider.imageUrls.length) {
        this.notification.warning("Vui lòng tải lên ảnh dịch vụ!");
        return;
      }
      if (this.flagState === 'new') {
        let objApiAdd = this.prepareModelBeforeSave();
        this._coreService
          .Post("create-service", objApiAdd)
          .subscribe((res) => {
      ...
      if (this.flagState === 'edit') {
        ...
        this._coreService
          .Post("update-service", objApiEdit)
          .subscribe((res) => {
