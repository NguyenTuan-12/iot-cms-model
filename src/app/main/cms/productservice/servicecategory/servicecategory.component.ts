...
@Component({
  selector: "cms-app-servicecategory",
...
export class ServiceCategoryComponent implements OnInit {
  ...
  viewRecord = (e) => {
    ...
    window.open("/cms/productservice/servicecategory/" + paramAdd, "_blank");
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(this.state, "get-list-servicecategory", extraParams);
  };
  public dataStateChange(state: DataStateChangeEventArgs): void {
    ...
    this._coreService.execute(state, "get-list-servicecategory", extraParams);
  }
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/productservice/servicecategory/new"]);
        break;
      default:
        break;
    }
      ...
      if (lstDeleteIds && lstDeleteIds.length > 0) {
        this._coreService
          ...
          .subscribe((success) => {
            ...
            if (x && x.errorCode == "ERROR_CATEGORY_HAS_DATA") {
              this.notification.warning("Không được phép xóa nhóm dịch vụ đã tồn tại trong danh sách dịch vụ");
            } else {
              this.notification.deleteError();
            }
          });
  ...
  clickRecord = (data, status) => {
    if (data && status === 'view') {
      ...
      const paramAdd = window.btoa(JSON.stringify(objParamAdd));
      // window.open("/cms/productservice/servicecategory/" + paramAdd, "_blank");
      this.router.navigate(["/cms/productservice/servicecategory", paramAdd]);
